...
#ifndef MLIR_DIALECT_AFIR_UTILS_SHAPEUTILS_H
#define MLIR_DIALECT_AFIR_UTILS_SHAPEUTILS_H
...
struct ValueKnowledge {
  ...
  ValueKnowledge(bool hasRank, llvm::ArrayRef<int64_t> newSizes, Type dtype)
    ...
    for (auto size : newSizes) sizes.push_back(size);
  }
  ...
  static ValueKnowledge getKnowledgeFromType(Type type) {
    ...
    if (auto shapedType = dyn_cast<ShapedType>(type)) {
      if (shapedType.hasRank()) {
        ...
        for (auto dim : shapedType.getShape()) result.sizes.push_back(dim);
      }
  ...
  Type getType() const {
    if (hasRank) return RankedTensorType::get(llvm::ArrayRef(sizes), dtype);
  ...
  static ValueKnowledge join(const ValueKnowledge &lhs, const ValueKnowledge &rhs) {
    ...
    if (!lhs || !rhs || lhs.dtype != rhs.dtype) return result;

    result.hasError = false;
    result.dtype = lhs.dtype;

    if (!lhs.hasRank && !rhs.hasRank) return result;
    ...
    if (lhs.sizes.size() != rhs.sizes.size()) return result;
...
#endif  // MLIR_DIALECT_AFIR_UTILS_SHAPEUTILS_H
