...
//   out_a[i,j] = max(t[i,j], 0)        out_a : [32,16] f16   (relu)
//   out_b[i,j] = t[i,j] * 2            out_b : [32,16] f16
//
...
//
// On-chip: the transpose loads its (row-strided) x slice into VECIN with a
// per-row DataCopy, AscendC::Transpose rearranges it into the output layout in
// a VECCALC TBuf, and both consumers read that same TBuf (it stays on-chip —
// not accumulated into a full-shape tensor that would bufferize to a GM
// out-param).  f16 + a square 16x16 inner tile (AscendC::Transpose basic form
// is 16-bit / 16x16).  d1 = 16 = full → not block-split with XBLOCK > 16, so
// block_dim = ceil(16 / XBLOCK); run.sh pins XBLOCK = XBLOCK_SUB = XBLOCK_X_0 =
// 16 → block_dim = 1.

#id2 = affine_map<(d0, d1) -> (d0, d1)>

module {
  func.func @transpose_preserve(%x: tensor<16x32xf16>)
    ...
    %b = linalg.generic {
        indexing_maps = [#id2, #id2],
        iterator_types = ["parallel", "parallel"]}
        ins(%t : tensor<32x16xf16>) outs(%b_init : tensor<32x16xf16>) {
    ^bb0(%v: f16, %o: f16):
