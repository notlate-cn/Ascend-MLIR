...
extern "C" __global__ __aicore__ void relu_diag_generated_buffers(
  ...
  pipe.InitBuffer(outputQueue, 1, kTileElems * sizeof(half));
  pipe.InitBuffer(data0Queue, 1, kTileElems * sizeof(half));
  ...
  for (uint32_t col = 0; col < kM; col += kTileCols) {
    ...
    AscendC::DataCopyPad(outGlobal[rowStart * kM + col], ready, storeParams);
