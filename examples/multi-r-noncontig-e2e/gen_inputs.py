#!/usr/bin/env python3
"""Generate test inputs for multi-r-noncontig-e2e (displaced multi-reduce-axes).
...
A_DEFAULT, R1_DEFAULT, R2_DEFAULT = 16, 8, 32
SEED = 42
...
def main():
    ...
    x = (rng.random((args.r1, args.a, args.r2),
                    dtype=np.float32) * 4 - 2).astype(np.float32)
    expected = x.sum(axis=(0, 2)).astype(np.float32)
