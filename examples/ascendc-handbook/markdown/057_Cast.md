<!-- 原始URL: https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/900beta1/API/ascendcopapi/atlasascendc_api_07_0073.html -->
...
![](/doc_center/source/zh/CANNCommunityEdition/900beta1/API/ascendcopapi/figure/zh-cn_image_0000002520882994.png)
...
![](/doc_center/source/zh/CANNCommunityEdition/900beta1/API/ascendcopapi/figure/zh-cn_image_0000002520882992.png)
...
![](/doc_center/source/zh/CANNCommunityEdition/900beta1/API/ascendcopapi/figure/zh-cn_image_0000002552122975.png)

上图中S = 0，E = 127，M = 2-1 \+ 2-2，最终表示的结果为1.75。

  * 二进制的舍入规则和十进制类似，具体如下： 

![](/doc_center/source/zh/CANNCommunityEdition/900beta1/API/ascendcopapi/figure/zh-cn_image_0000002520882998.png)
...
---|---|---  
float |  float |  将src按照roundMode（精度转换处理模式，参见[参数说明](#ZH-CN_TOPIC_0000002520880724__section622mcpsimp)中的roundMode参数）取整，仍以float格式存入dst中。 示例：输入0.5， CAST_RINT模式输出0.0，CAST_FLOOR模式输出0.0，CAST_CEIL模式输出1.0，CAST_ROUND模式输出1.0，CAST_TRUNC模式输出0.0。  
half |  将src按照roundMode取到half所能表示的数，以half格式（溢出默认按照饱和处理）存入dst中。 示例：输入0.5 + 2-12，写成float的表示形式：2-1 * (1 + 2-11)，因此E = -1 + 127 = 126，M = 2-11。 ![](/doc_center/source/zh/CANNCommunityEdition/900beta1/API/ascendcopapi/figure/zh-cn_image_0000002552122979.png) half的指数位可以表示出2-1，E = -1 + 15 = 14，但half只有10 bit尾数位，因此灰色部分要进行舍入。 CAST_RINT模式舍入得尾数0000000000，E = 14，M = 0，最终表示的结果为0.5； CAST_FLOOR模式舍入得尾数0000000000，E = 14，M = 0，最终表示的结果为0.5； CAST_CEIL模式舍入得尾数0000000001，E = 14，M = 2-10，最终表示的结果为0.5 + 2-11； CAST_ROUND模式舍入得尾数0000000001，E = 14，M = 2-10，最终表示的结果为0.5 + 2-11； CAST_TRUNC模式舍入得尾数0000000000，E = 14，M = 0，最终表示的结果为0.5； CAST_ODD模式舍入得尾数0000000001，E = 14，M = 2-10，最终表示的结果为0.5 + 2-11 。  
...
half |  与[SetDeqScale(half scale)](atlasascendc_api_07_0099.html)接口配合使用，输出src / 217 * scale * 217。  
...
参数名 |  描述  
---|---  
T |  目的操作数数据类型。 Atlas A3 训练系列产品 / Atlas A3 推理系列产品 ，支持的数据类型见[表7](#ZH-CN_TOPIC_0000002520880724__table52491124113511) Atlas A2 训练系列产品 / Atlas A2 推理系列产品 ，支持的数据类型见[表6](#ZH-CN_TOPIC_0000002520880724__table588610209212) Atlas 200I/500 A2 推理产品 ，支持的数据类型见[表8](#ZH-CN_TOPIC_0000002520880724__table59169016488) Atlas 推理系列产品 AI Core，支持的数据类型见[表5](#ZH-CN_TOPIC_0000002520880724__table1749772361915) Atlas 训练系列产品 ，支持的数据类型见[表4](#ZH-CN_TOPIC_0000002520880724__table18921155033213)  
U |  源操作数数据类型。 Atlas A3 训练系列产品 / Atlas A3 推理系列产品 ，支持的数据类型见[表7](#ZH-CN_TOPIC_0000002520880724__table52491124113511) Atlas A2 训练系列产品 / Atlas A2 推理系列产品 ，支持的数据类型见[表6](#ZH-CN_TOPIC_0000002520880724__table588610209212) Atlas 200I/500 A2 推理产品 ，支持的数据类型见[表8](#ZH-CN_TOPIC_0000002520880724__table59169016488) Atlas 推理系列产品 AI Core，支持的数据类型见[表5](#ZH-CN_TOPIC_0000002520880724__table1749772361915) Atlas 训练系列产品 ，支持的数据类型见[表4](#ZH-CN_TOPIC_0000002520880724__table18921155033213)  
...
src |  输入 |  源操作数。 类型为[LocalTensor](atlasascendc_api_07_0006.html)，支持的TPosition为VECIN/VECCALC/VECOUT。 LocalTensor的起始地址需要32字节对齐。  
roundMode |  输入 |  精度转换处理模式，类型是RoundMode。 RoundMode为枚举类型，用以控制精度转换处理模式，具体定义为：
...
对于 Atlas 训练系列产品 ，CAST_ROUND表示反向0取整，远离0，对正数x.y变成(x + 1)，对负数-x.y，变成-(x + 1)。  
count |  输入 |  参与计算的元素个数。  
...
repeatTime |  输入 |  重复迭代次数。矢量计算单元，每次读取连续的256Bytes数据进行计算，为完成对输入数据的处理，必须通过多次迭代（repeat）才能完成所有数据的读取与计算。repeatTime表示迭代的次数，repeatTime∈[0,255]。 关于该参数的具体描述请参考[高维切分API](/document/detail/zh/CANNCommunityEdition/900beta1/opdevg/Ascendcopdevg/atlas_ascendc_10_0022.html)。  
repeatParams |  输入 |  控制操作数地址步长的参数，[UnaryRepeatParams](atlasascendc_api_07_0012.html)类型。包含操作数相邻迭代间的地址步长，操作数同一迭代内datablock的地址步长等参数。其中dstRepStride/srcRepStride∈[0,255]。 相邻迭代间的地址步长参数说明请参考[repeatStride](/document/detail/zh/CANNCommunityEdition/900beta1/opdevg/Ascendcopdevg/atlas_ascendc_10_0022.html#ZH-CN_TOPIC_0000002552129981__zh-cn_topic_0000002267504656_zh-cn_topic_0000001764162593_section139459347420)；同一迭代内DataBlock的地址步长参数说明请参考[dataBlockStride](/document/detail/zh/CANNCommunityEdition/900beta1/opdevg/Ascendcopdevg/atlas_ascendc_10_0022.html#ZH-CN_TOPIC_0000002552129981__zh-cn_topic_0000002267504656_zh-cn_topic_0000001764162593_section2815124173416)。  
  
表4 Atlas 训练系列产品 Cast指令参数说明
...
表5 Atlas 推理系列产品 AI CoreCast指令参数说明
...
int32_t |  float |  CAST_NONE  
int16_t |  CAST_NONE  
half |  roundMode不生效，与[SetDeqScale(half scale)](atlasascendc_api_07_0099.html)接口配合使用。  
  
表6 Atlas A2 训练系列产品 / Atlas A2 推理系列产品 Cast指令参数说明
...
表7 Atlas A3 训练系列产品 / Atlas A3 推理系列产品 Cast指令参数说明
...
表8 Atlas 200I/500 A2 推理产品 Cast指令参数说明
...
#### 约束说明
  ...
  * 操作数地址重叠约束请参考[通用地址重叠约束](atlasascendc_api_07_0004.html#ZH-CN_TOPIC_0000002552120513__section668772811100)。特别地，对于长度较小的数据类型转换为长度较大的数据类型时，地址重叠可能会导致结果错误。
...
如果您需要运行样例代码，请将该代码段拷贝并替换[样例模板](#ZH-CN_TOPIC_0000002520880724__section12233154963115)中Compute函数的部分代码即可。
...
**父主题：** [类型转换](atlasascendc_api_07_0072.html)
