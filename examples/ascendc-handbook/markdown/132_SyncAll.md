<!-- 原始URL: https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/900beta1/API/ascendcopapi/atlasascendc_api_07_0204.html -->
...
参数名称 | 输入/输出 | 含义  
---|---|---  
gmWorkspace | 输入 | gmWorkspace为用户定义的全局Global空间，作为所有核共用的缓存，用于保存每个核的状态标记，类型为GlobalTensor，支持的数据类型为int32_t。GlobalTensor数据结构的定义请参考[GlobalTensor](atlasascendc_api_07_0007.html)。 所需空间大小和使用注意项参见[约束说明](#ZH-CN_TOPIC_0000002552080057__section633mcpsimp)。 硬同步接口不支持该参数。  
ubWorkspace | 输入 | ubWorkspace为用户定义的局部Local空间，每个核单独自用，用于标记当前核的状态。 类型为[LocalTensor](atlasascendc_api_07_0006.html)，支持的TPosition为VECIN/VECCALC/VECOUT，支持的数据类型为int32_t。 所需空间大小参见[约束说明](#ZH-CN_TOPIC_0000002552080057__section633mcpsimp)。 硬同步接口不支持该参数。  
