<!-- 原始URL: https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/900beta1/API/ascendcopapi/atlasascendc_api_07_0071.html -->
...
U | 用户自定义模式下src1Pattern的数据类型。支持的数据类型为uint16_t/uint32_t。
...
src0 | 输入 | 源操作数。 类型为[LocalTensor](atlasascendc_api_07_0006.html)，支持的TPosition为VECIN/VECCALC/VECOUT。 LocalTensor的起始地址需要32字节对齐。 数据类型需要与目的操作数保持一致。  
src1Pattern | 输入 | gather mask（数据收集的掩码），分为内置固定模式和用户自定义模式两种，根据内置固定模式对应的二进制或者用户自定义输入的Tensor数值对应的二进制从源操作数中选取元素写入目的操作数中。1为选取，0为不选取。
...
Atlas A3 训练系列产品/Atlas A3 推理系列产品支持模式1-7 Atlas A2 训练系列产品/Atlas A2 推理系列产品支持模式1-7 Atlas 200I/500 A2 推理产品支持模式1-7 Atlas 推理系列产品AI Core支持模式1-6


  * 用户自定义模式：src1Pattern数据类型为LocalTensor，迭代间间隔由src1RepeatStride决定， 迭代内src1Pattern连续消耗。

  
reduceMode | 输入 | 用于选择mask参数模式，数据类型为bool，支持如下取值。
  ...
  * true：Counter模式。根据mask等参数含义的不同，该模式有以下两种配置方式：
    * [配置方式一](#ZH-CN_TOPIC_0000002552080339__fig126588515614)：每次repeat操作mask个元素，总的数据计算量为repeatTimes * mask个元素。
      * mask值配置为每一次repeat计算的元素个数。
      * 按需配置repeatTimes、src0BlockStride、src0RepeatStride参数。
      * 支持src1Pattern配置为内置固定模式或用户自定义模式。用户自定义模式下可根据实际情况配置src1RepeatStride。
    * [配置方式二](#ZH-CN_TOPIC_0000002552080339__fig13219248171214)：总的数据计算量为mask个元素。
      * mask配置为总的数据计算量。
...
mask | 输入 | 用于控制每次迭代内参与计算的元素。根据reduceMode，分为两种模式：

  * Normal模式：mask无效，建议设置为0。
  * Counter模式：取值范围[1, 232 – 1]。不同的版本型号Counter模式下，mask参数表示含义不同。具体配置规则参考上文reduceMode参数描述。

  
gatherMaskParams | 输入 | 控制操作数地址步长的数据结构，GatherMaskParams类型。 具体定义请参考${INSTALL_DIR}/include/ascendc/basic_api/interface/kernel_struct_gather.h，${INSTALL_DIR}请替换为CANN软件安装后文件存储路径。 具体参数说明[表3](#ZH-CN_TOPIC_0000002552080339__table1940815635619)。  
...
![](/doc_center/source/zh/CANNCommunityEdition/900beta1/API/ascendcopapi/figure/zh-cn_image_0000002520882600.png)
...
![](/doc_center/source/zh/CANNCommunityEdition/900beta1/API/ascendcopapi/figure/zh-cn_image_0000002552122581.png)
