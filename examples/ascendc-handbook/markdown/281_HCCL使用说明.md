<!-- 原始URL: https://www.hiascend.com/document/detail/zh/CANNCommunityEdition/900beta1/API/ascendcopapi/atlasascendc_api_07_0867.html -->
...
Ascend C提供一组HCCL通信类高阶API，方便算子Kernel开发用户在AI Core侧灵活管理通算融合算子中计算与通信任务的执行顺序。使用HCCL通信类高阶API进行算子开发前，请参考[通算融合](/document/detail/zh/CANNCommunityEdition/900beta1/opdevg/Ascendcopdevg/atlas_ascendc_10_10033.html)章节了解必要的背景知识。

HCCL为**集合通信任务客户端** ，主要对外提供了集合通信原语接口（以下统称为Prepare接口），对标集合通信C++接口，详细可参见[HCCL接口参考](/document/detail/zh/CANNCommunityEdition/900beta1/API/hcclapiref/hcclapi_07_0001.html)，当前支持[AllReduce](atlasascendc_api_07_0872.html)、[AllGather](atlasascendc_api_07_0873.html)、[ReduceScatter](atlasascendc_api_07_0874.html)、[AlltoAll](atlasascendc_api_07_0875.html)接口等。本章的所有接口运行在AI Core上，且不执行通信任务，而是由用户调用Prepare接口将对应类型的通信任务信息发送给AI CPU服务端，并在合适的时机通过[Commit](atlasascendc_api_07_0877.html)接口通知AI CPU上的服务端执行对应的通信任务。

所谓合适的时机，取决于用户编排的是先通信后计算的任务，还是先计算后通信的任务。对于这两种场景，简述如下：

  * 先通信后计算的任务：典型的如AllGather通信+Matmul计算任务编排。此场景下，用户在调用AllGather接口下发通信任务之后，通过AllGather接口返回的该通信任务标识handleId，可立即调用Commit接口通知服务端执行该handleId对应的任务，同时用户调用[Wait](atlasascendc_api_07_0878.html)阻塞接口等待服务端通知handleId对应的通信任务执行结束，待该通信任务结束后，再执行计算任务。
...
当后续无通信任务时，调用[Finalize](atlasascendc_api_07_0881.html)接口，通知服务端后续无通信任务，执行结束后退出，客户端检测并等待最后一个通信任务执行结束。以上介绍的AI Core下发HCCL通信任务的机制如下图所示。

**图1** AI Core下发HCCL通信任务机制   


![](/doc_center/source/zh/CANNCommunityEdition/900beta1/API/ascendcopapi/figure/zh-cn_image_0000002520881298.png)

注意

对于 Atlas A3 训练系列产品 / Atlas A3 推理系列产品 ，在AI CPU作为服务端的场景中，HCCL通信API的功能依赖开放AI CPU用户态下发调度任务，存在一定的安全风险，用户需要自行确保AI Core自定义算子的安全可靠，防止恶意攻击行为。

实现AI Core下发一个通信任务的具体步骤如下：

  1. 创建HCCL对象，并调用初始化接口[InitV2](atlasascendc_api_07_10221.html)。
...
当调用InitV2接口时，必须使用标准C++语法定义TilingData结构体的开发方式，具体请参考[使用标准C++语法定义Tiling结构体](/document/detail/zh/CANNCommunityEdition/900beta1/opdevg/Ascendcopdevg/atlas_ascendc_10_00024.html)。如上示例代码中的tilingGM为host侧传入的、作为核函数入参的算子TilingData的GM地址，通过[GET_TILING_DATA_WITH_STRUCT](atlasascendc_api_07_0215.html)获取TilingData。调用[InitV2](atlasascendc_api_07_10221.html)初始化接口时，需要传入通信上下文信息，可以通过框架提供的获取通信上下文的接口[GetHcclContext](atlasascendc_api_07_1219.html)获取。

  2. 设置对应通信算法的Tiling地址。

通过[SetCcTilingV2](atlasascendc_api_07_10222.html)接口设置对应通信算法的Tiling地址，调用Commit接口后，该地址被发送到服务端由服务端解析。[SetCcTilingV2](atlasascendc_api_07_10222.html)接口必须与[InitV2](atlasascendc_api_07_10221.html)接口配合使用。示例如下。 
...
示例的Prepare接口为[ReduceScatter](atlasascendc_api_07_0874.html)，其他接口可参考后续章节的内容。其中的参数AscendC::HCCL_DATA_TYPE_FP16是HCCL任务的数据类型，其数据结构为HcclDataType，对应的参数说明参考[表1](#ZH-CN_TOPIC_0000002521039850__table116710585514)；参数HCCL_REDUCE_SUM是一种Reduce操作，AllReduce和ReduceScatter归约操作支持的Reduce操作类型参见[表2](#ZH-CN_TOPIC_0000002521039850__table2469980529)。
...
HcclReduceOp |  Reduce操作类型。
  ...
  4. 用户调用[Commit](atlasascendc_api_07_0877.html)接口通知服务端执行handleId对应的通信任务。
         
         // 等待通信任务执行时机成熟，调用Commit接口通知服务端执行
         hccl.Commit(handleId);
         

  5. 用户调用[Wait](atlasascendc_api_07_0878.html)阻塞接口，等待服务端执行完对应的通信任务。
  ...
  6. 用户调用[Finalize](atlasascendc_api_07_0881.html)接口，通知服务端后续无通信任务，执行结束后退出；客户端检测并等待最后一个通信任务执行结束。
...
基于以上对单个通信任务下发的了解，介绍Prepare接口中repeat参数的灵活使用方式。一次Prepare接口的调用对应一个handleId，Prepare接口中的参数repeat代表这次Prepare的通信任务次数，该值必须和针对该handleId调用Commit接口的次数、Wait接口的次数一致。以[图2 ReduceScatter通信示例](#ZH-CN_TOPIC_0000002521039850__fig19780192164210)进行说明，假设共4张卡，每张卡上源数据首先按照rankId均匀分成4份，每份数据被切分成3份，最终被切分后的每份数据的个数为TileLen，每次ReduceScatter通信仅通信一组切分数据（如图中数据0-0、1-0、2-0、3-0为一组切分数据），因此需要做3次ReduceScatter操作，全部数据才能通信完。
...
![](/doc_center/source/zh/CANNCommunityEdition/900beta1/API/ascendcopapi/figure/zh-cn_image_0000002520881286.png)
...
提示：调试含有HCCL高阶API的算子时，在算子编译工程中增加编译选项[-DASCENDC_DEBUG](/document/detail/zh/CANNCommunityEdition/900beta1/opdevg/Ascendcopdevg/atlas_ascendc_10_0068.html#ZH-CN_TOPIC_0000002520889960__table151052168302)，可以使能异常场景拦截的能力，具体内容请参考并使用[assert接口](atlasascendc_api_07_0194.html)。

**父主题：** [HCCL Kernel侧接口](atlasascendc_api_07_0868.html)
