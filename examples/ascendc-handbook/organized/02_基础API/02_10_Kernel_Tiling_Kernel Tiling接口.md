...
使用该接口指定结构体名称，可获取指定的tiling信息，并填入对应的Tiling结构体中，此函数会以宏展开的方式进行编译。与[GET_TILING_DATA](atlasascendc_api_07_0214.html)的区别是：[GET_TILING_DATA](atlasascendc_api_07_0214.html)只能获取默认注册的结构体，该接口可以根据指定的结构体名称获取对应的结构体，常用于针对不同的TilingKey注册了不同结构体的情况下。
...
在Kernel侧使用标准C++语法自定义的TilingData结构体时，若用户不确定需要注册哪些结构体，可使用该接口告知框架侧需使用未注册的标准C++语法来定义TilingData，并配套[GET_TILING_DATA_WITH_STRUCT](atlasascendc_api_07_0215.html)，[GET_TILING_DATA_MEMBER](atlasascendc_api_07_0216.html)，[GET_TILING_DATA_PTR_WITH_STRUCT](atlasascendc_api_07_00140.html)来获取对应的TilingData。
...
#### 约束说明
  ...
  * 暂不支持Kernel直调工程。
  * 使用[GET_TILING_DATA](atlasascendc_api_07_0214.html)需提供默认注册的TilingData结构体，但本接口不注册TilingData结构体，故不支持与[5.11.1-GET_TILING_DATA](atlasascendc_api_07_0214.html)组合使用。
  * 不支持和[REGISTER_TILING_DEFAULT](atlasascendc_api_07_00003.html)或[REGISTER_TILING_FOR_TILINGKEY](atlasascendc_api_07_00004.html)混用，即不支持注册TilingData结构体的场景与非注册场景混合使用。
...
KERNEL_TYPE_MIX_AIV_1_0 |  AIC、AIV混合场景下，使用了[多核控制相关指令](atlasascendc_api_07_0268.html)时，设置核函数的类型为MIX AIV:AIC 1:0（带有硬同步），算子执行时仅会启动AI Core上的Vector核，比如用户在host侧设置numBlocks为10，则会启动10个Vector核。 硬同步的概念解释如下：当不同核之间操作同一块全局内存且可能存在读后写、写后读以及写后写等数据依赖问题时，通过调用[SyncAll()](atlasascendc_api_07_0204.html)函数来插入同步语句来避免上述数据依赖时可能出现的数据读写错误问题。目前多核同步分为硬同步和软同步，硬同步是利用硬件自带的全核同步指令由硬件保证多核同步。  
KERNEL_TYPE_MIX_AIC_1_0 |  AIC、AIV混合场景下，使用了[多核控制相关指令](atlasascendc_api_07_0268.html)时，设置核函数的类型为MIX AIC:AIV 1:0（带有硬同步），算子执行时仅会启动AI Core上的Cube核，比如用户在host侧设置numBlocks为10，则会启动10个Cube核。  
...
KERNEL_TYPE_MIX_VECTOR_CORE |  基于Ascend C开发的矢量计算相关的算子可以运行在Vector Core上，调用本接口传入该参数用于使能Vector Core。 使能Vector Core后，算子执行时会同时启动AI Core和Vector Core，用于并行计算。比如用户在host侧设置numBlocks为10，则会启动总数为10的AI Core和Vector Core。 需要注意的是，通过[SetBlockDim](/document/detail/zh/CANNCommunityEdition/900beta1/API/basicdataapi/atlasopapi_07_00236.html)设置核数时，需要大于AI Core的核数，否则不会启动VectorCore。  
...
#### 约束说明
  ...
  * 使用[Tiling模板编程](/document/detail/zh/CANNCommunityEdition/900beta1/opdevg/Ascendcopdevg/atlas_ascendc_10_00025.html)时，需要通过ASCENDC_TPL_KERNEL_TYPE_SEL设置Kernel类型即可，无需再通过该接口进行设置，本接口不生效。
...
#### 调用示例
  ...
  * 示例一：使能VectorCore样例 
    ...
    2. 完成算子host侧Tiling开发时，设置的numBlocks代表的是AI Core和Vector Core的总数，比如用户在host侧设置numBlocks为10，则会启动总数为10的AI Core和Vector Core；为保证启动Vector Core，设置数值应大于AI Core的核数。您可以通过[GetCoreNumAic](atlasascendc_api_07_1030.html)接口获取AI Core的核数，[GetCoreNumVector](atlasascendc_api_07_1032.html)接口获取Vector Core的核数。 如下代码片段，展示了numBlocks的设置方法，此处设置为AI Core和Vector Core的总和，表示所有AI Core和Vector Core都启动。 
