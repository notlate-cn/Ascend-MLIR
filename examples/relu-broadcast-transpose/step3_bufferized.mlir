  ...
  func.func @ewop_broadcast_transpose(%arg0: memref<?x?xf16>, %arg1: memref<?xf16>, %arg2: memref<?xf16>, %arg3: i64, %arg4: i64) -> memref<?x?xf16> {
    ...
    %2 = scf.for %arg5 = %c0 to %dim step %1 iter_args(%arg6 = %alloc) -> (memref<?x?xf16>) {
      ...
      %6 = scf.for %arg7 = %c0 to %5 step %0 iter_args(%arg8 = %subview_8) -> (memref<?x?xf16, strided<[?, 1], offset: ?>>) {
        ...
        %subview_10 = memref.subview %subview_7[0] [%dim_0] [1] : memref<?xf16, strided<[1]>> to memref<?xf16, strided<[1]>>
        %subview_11 = memref.subview %arg8[%arg7, 0] [%7, %dim_0] [1, 1] : memref<?x?xf16, strided<[?, 1], offset: ?>> to memref<?x?xf16, strided<[?, 1], offset: ?>>
        linalg.generic {indexing_maps = [#map1, #map2, #map1], iterator_types = ["parallel", "parallel"], library_call = "relu_bias_add"} ins(%subview_9, %subview_10 : memref<?x?xf16, strided<[?, 1], offset: ?>>, memref<?xf16, strided<[1]>>) outs(%subview_11 : memref<?x?xf16, strided<[?, 1], offset: ?>>) attrs =  {ascendc.unit = "AiCore.Vector"} {
        ^bb0(%in: f16, %in_12: f16, %out: f16):
          %8 = arith.maximumf %in, %cst : f16
          %9 = arith.addf %8, %in_12 : f16
          linalg.yield %9 : f16
        }
    ...
    %3 = scf.for %arg5 = %c0 to %dim_3 step %1 iter_args(%arg6 = %alloc_4) -> (memref<?x?xf16>) {
      ...
      %6 = scf.for %arg7 = %c0 to %5 step %0 iter_args(%arg8 = %subview_7) -> (memref<?x?xf16, strided<[?, 1], offset: ?>>) {
        ...
        %subview_9 = memref.subview %arg8[%arg7, 0] [%7, %dim_2] [1, 1] : memref<?x?xf16, strided<[?, 1], offset: ?>> to memref<?x?xf16, strided<[?, 1], offset: ?>>
        linalg.generic {indexing_maps = [#map3, #map1], iterator_types = ["parallel", "parallel"], library_call = "transpose"} ins(%subview_8 : memref<?x?xf16, strided<[?, 1], offset: ?>>) outs(%subview_9 : memref<?x?xf16, strided<[?, 1], offset: ?>>) attrs =  {ascendc.unit = "AiCore.Vector"} {
        ^bb0(%in: f16, %out: f16):
          linalg.yield %in : f16
        }
        memref.copy %subview_9, %subview_9 : memref<?x?xf16, strided<[?, 1], offset: ?>> to memref<?x?xf16, strided<[?, 1], offset: ?>>
        scf.yield %arg8 : memref<?x?xf16, strided<[?, 1], offset: ?>>
      } {ascendc.epilogue = "dst:VECOUT->GM", ascendc.prologue = "src:GM->VECIN"}
    ...
    %4 = scf.for %arg5 = %c0 to %dim_5 step %1 iter_args(%arg6 = %alloc_1) -> (memref<?x?xf16>) {
      ...
      %6 = scf.for %arg7 = %c0 to %5 step %0 iter_args(%arg8 = %subview_8) -> (memref<?x?xf16, strided<[?, 1], offset: ?>>) {
        ...
        %subview_10 = memref.subview %subview_7[0] [%dim_6] [1] : memref<?xf16, strided<[1]>> to memref<?xf16, strided<[1]>>
        %subview_11 = memref.subview %arg8[%arg7, 0] [%7, %dim_6] [1, 1] : memref<?x?xf16, strided<[?, 1], offset: ?>> to memref<?x?xf16, strided<[?, 1], offset: ?>>
        linalg.generic {indexing_maps = [#map1, #map2, #map1], iterator_types = ["parallel", "parallel"], library_call = "scale_mul"} ins(%subview_9, %subview_10 : memref<?x?xf16, strided<[?, 1], offset: ?>>, memref<?xf16, strided<[1]>>) outs(%subview_11 : memref<?x?xf16, strided<[?, 1], offset: ?>>) attrs =  {ascendc.unit = "AiCore.Vector"} {
        ^bb0(%in: f16, %in_12: f16, %out: f16):
          %8 = arith.mulf %in, %in_12 : f16
          linalg.yield %8 : f16
        }
        memref.copy %subview_11, %subview_11 : memref<?x?xf16, strided<[?, 1], offset: ?>> to memref<?x?xf16, strided<[?, 1], offset: ?>>
        scf.yield %arg8 : memref<?x?xf16, strided<[?, 1], offset: ?>>
