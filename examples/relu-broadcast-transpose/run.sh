...
echo ""
echo "  [计算图结构]"
echo "    Op1 relu_bias_add:  iterator = [Parallel, Parallel], d0=M, d1=N"
echo "         relu(A[M,N]) + broadcast_col(bias[N]) → B[M,N]"
echo "    Op2 transpose:      iterator = [Parallel, Parallel], d0=N, d1=M"
echo "         B[M,N] → C[N,M]（ins 访问 (d1,d0)，即 B[j,i]）"
echo "    Op3 scale_mul:      iterator = [Parallel, Parallel], d0=N, d1=M"
echo "         C[N,M] * broadcast_col(scale[M]) → D[N,M]"

# ── STAGE 1: 融合 ──────────────────────────────────────────
echo ""
echo "[STAGE 1] 融合：--linalg-fuse-elementwise-ops"
echo "  Op1 与 Op2/Op3 之间存在 transpose 依赖，通常无法融合"
...
echo "  策略: 对 Op1/Op2/Op3 分别沿第一轴（M 或 N）做两级切分 TB/Tb"
...
echo "  转换规则："
echo "    Op1 (relu+broadcast_col+add):"
echo "       → broadcast_l2（列广播 bias）+ duplicate_l2（zero）+ max_l2（relu）+ add_l2"
echo "    Op2 (transpose):"
echo "       → ascendc.transpose %dst, %src（检测到置换 indexing_map）"
echo "    Op3 (scale_mul):"
...
grep -E "ascendc\.(broadcast_l2|add_l2|mul_l2|max_l2|transpose|data_copy)" \
