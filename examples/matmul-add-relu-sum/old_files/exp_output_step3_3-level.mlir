...
module {
  func.func @fc_relu(
      %metrix_A: tensor<?x?xf32>,  // A [M, K]
      %metrix_B: tensor<?x?xf32>,  // B [K, N]
      %metrix_bias: tensor<?x?xf32>,  // bias [M, N]
      %metrix_output: tensor<?x?xf32>,  // output [M, N]
    ...
    // 全局维度
    %dim_M = tensor.dim %metrix_output, %c0 : tensor<?x?xf32>
    %dim_N = tensor.dim %metrix_output, %c1 : tensor<?x?xf32>
    %dim_K = tensor.dim %metrix_A, %c1 : tensor<?x?xf32>

    // 全零tensor: 用于 relu 的比较基准 (linalg.fill 在核外完成)
    %zero_buf    = tensor.empty(%dim_M, %dim_N) : tensor<?x?xf32>
    %metrix_zero = linalg.fill ins(%cst : f32) outs(%zero_buf : tensor<?x?xf32>) -> tensor<?x?xf32>
    ...
    %result = scf.forall (%iv_TB_M, %iv_TB_N)
                  ...
                  step (%TB_M, %TB_N)        // 每核步长 = TB tile size(TB_M, TB_N)
                  shared_outs(%out = %metrix_output)  // 输出张量
              -> tensor<?x?xf32> {
      ...
      %bias_TB = tensor.extract_slice %metrix_bias[%iv_TB_M, %iv_TB_N][%sz_TB_M, %sz_TB_N][1, 1] : tensor<?x?xf32> to tensor<?x?xf32>
      %zero_TB = tensor.extract_slice %metrix_zero[%iv_TB_M, %iv_TB_N][%sz_TB_M, %sz_TB_N][1, 1] : tensor<?x?xf32> to tensor<?x?xf32>
      // 本核输出 tile 初始值 (从 shared_outs 取出)
      %out_TB_init = tensor.extract_slice %out[%iv_TB_M, %iv_TB_N][%sz_TB_M, %sz_TB_N][1, 1] : tensor<?x?xf32> to tensor<?x?xf32>
      ...
      // =========================================================
      %out_Tb = scf.for %iv_Tb_M = %c0 to %sz_TB_M step %Tb_M iter_args(%acc_Tb = %out_TB_init) -> tensor<?x?xf32> {
        // 当前 Tb_M 的实际大小
        ...
        %A_L1  = tensor.extract_slice %metrix_A[%abs_M, 0][%sz_Tb_M, %dim_K][1, 1] : tensor<?x?xf32> to tensor<?x?xf32>
        ...
        // =========================================================
        %out_Tb_N = scf.for %iv_Tb_N = %c0 to %sz_TB_N step %Tb_N iter_args(%acc_Tb_N = %acc_Tb) -> tensor<?x?xf32> {
          // 当前 Tb_N 的实际大小
          ...
          %B_L1  = tensor.extract_slice %metrix_B[0, %abs_N][%dim_K, %sz_Tb_N][1, 1] : tensor<?x?xf32> to tensor<?x?xf32>

          // 当前 (Tb_M x Tb_N) 输出 tile (从累加张量中取出)
          %C_Tb_init = tensor.extract_slice %acc_Tb_N[%iv_Tb_M, %iv_Tb_N][%sz_Tb_M, %sz_Tb_N][1, 1] : tensor<?x?xf32> to tensor<?x?xf32>
          ...
          // =====================================================
          %C_Tb_matmul = scf.for %iv_K = %c0 to %dim_K step %t_K iter_args(%C_acc = %C_Tb_init) -> tensor<?x?xf32> {
            %sz_K = affine.min #map(%iv_K)[%dim_K, %t_K]

            // L0A: A_L1 的 K 切片 [0:sz_Tb_M, iv_K:sz_K]
            %A_L0 = tensor.extract_slice %A_L1[0, %iv_K][%sz_Tb_M, %sz_K][1, 1] : tensor<?x?xf32> to tensor<?x?xf32>
            // L0B: B_L1 的 K 切片 [iv_K:sz_K, 0:sz_Tb_N]
            %B_L0 = tensor.extract_slice %B_L1[%iv_K, 0][%sz_K, %sz_Tb_N][1, 1] : tensor<?x?xf32> to tensor<?x?xf32>
            // L0C: 当前累加结果
            %C_L0 = tensor.extract_slice %C_acc[0, 0][%sz_Tb_M, %sz_Tb_N][1, 1] : tensor<?x?xf32> to tensor<?x?xf32>
            ...
            %C_acc_new = tensor.insert_slice %C_L0_out into %C_acc [0, 0][%sz_Tb_M, %sz_Tb_N][1, 1] : tensor<?x?xf32> into tensor<?x?xf32>
          ...
          // -------------------------------------------------
          %bias_Tb = tensor.extract_slice %bias_TB[%iv_Tb_M, %iv_Tb_N][%sz_Tb_M, %sz_Tb_N][1, 1] : tensor<?x?xf32> to tensor<?x?xf32>
          %out_for_add = tensor.extract_slice %acc_Tb_N[%iv_Tb_M, %iv_Tb_N][%sz_Tb_M, %sz_Tb_N][1, 1] : tensor<?x?xf32> to tensor<?x?xf32>
          ...
          // -------------------------------------------------
          %zero_Tb = tensor.extract_slice %zero_TB[%iv_Tb_M, %iv_Tb_N][%sz_Tb_M, %sz_Tb_N][1, 1] : tensor<?x?xf32> to tensor<?x?xf32>
          %out_for_relu = tensor.extract_slice %acc_Tb_N[%iv_Tb_M, %iv_Tb_N][%sz_Tb_M, %sz_Tb_N][1, 1] : tensor<?x?xf32> to tensor<?x?xf32>
          ...
          // Tb tile 结果写回核内累加张量
          %acc_Tb_N_new = tensor.insert_slice %relu_result into %acc_Tb_N[%iv_Tb_M, %iv_Tb_N][%sz_Tb_M, %sz_Tb_N][1, 1] : tensor<?x?xf32> into tensor<?x?xf32>
      ...
      scf.forall.in_parallel {
        tensor.parallel_insert_slice %out_Tb into %out[%iv_TB_M, %iv_TB_N][%sz_TB_M, %sz_TB_N][1, 1] : tensor<?x?xf32> into tensor<?x?xf32>
      }
    } // end scf.forall (TB)
