"""Generate input/expected-output npy files for fc_leakyrelu_mix kernel.

Computation: output[m,n] = leakyrelu(A[m,k] @ B[k,n] + bias[n], alpha=0.001)
  A:    (M, K)  float16
  B:    (K, N)  float16
...
Usage:
  python3 gen_data_leakyrelu.py [--M M] [--K K] [--N N] [--seed SEED] [--out-dir DIR]
"""
...
def main():
    ...
    # Reference: compute in float32 for accuracy
    matmul = (A.astype(np.float32) @ B.astype(np.float32))  # [M, N]
    added  = matmul + bias                                    # broadcast bias[N] over M
    alpha  = 0.001
