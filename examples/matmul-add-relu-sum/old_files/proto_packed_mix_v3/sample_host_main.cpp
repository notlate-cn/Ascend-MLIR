...
int main() {
  ...
  auto T = readFile("/Users/niu/code/samples/operator/ascendc/0_introduction/22_baremix_kernellaunch/BareMixInvocation/input/tiling.bin");
  ...
  CHECK_ACL(aclrtMallocHost((void**)&outHost, 16384 * sizeof(float)));
  ...
  CHECK_ACL(aclrtMalloc((void**)&outDev, 16384 * sizeof(float), ACL_MEM_MALLOC_HUGE_FIRST));
  ...
  uint32_t rc = aclrtlaunch_baremix_custom(1, stream, aDev, bDev, biasDev, outDev, workspaceDev, tilingDev);
  std::cout << "launch_rc=" << rc << "\n";
  CHECK_ACL(aclrtSynchronizeStream(stream));
  int d2h_rc = aclrtMemcpy(outHost, 16384 * sizeof(float), outDev, 16384 * sizeof(float), ACL_MEMCPY_DEVICE_TO_HOST);
