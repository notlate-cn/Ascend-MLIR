...
#include <cstring>
#include "acl/acl.h"
extern "C" uint32_t aclrtlaunch_fc_leakyrelu(uint32_t numBlocks, aclrtStream stream,
...
int main(){
  auto A=readFile("/tmp/packed_raw_inputs/fc_leakyrelu_input_a.bin");
  auto B=readFile("/tmp/packed_raw_inputs/fc_leakyrelu_input_b.bin");
  auto Bias=readFile("/tmp/packed_raw_inputs/fc_leakyrelu_input_bias.bin");
  auto T=readFile("/tmp/fc_leakyrelu_tiling.bin");
  ...
  int d2h_rc=aclrtMemcpy(outHost,128*128*4,outDev,128*128*4,ACL_MEMCPY_DEVICE_TO_HOST); std::cout << "d2h_rc=" << d2h_rc << "\n";
