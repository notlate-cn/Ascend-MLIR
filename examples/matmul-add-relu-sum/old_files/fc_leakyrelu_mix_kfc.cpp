// Trial variant: keep working fc_relu_mix KFC skeleton, change only activation to LeakyRelu.
...
extern "C" __global__ __aicore__ void auto_gen_fc_leakyrelu_kfc_kernel(
    ...
    GM_ADDR out, GM_ADDR workspace, GM_ADDR tilingGm)
{
    ...
    for (int32_t row = 0; row < M_; row += baseM_) {
        ...
        LeakyRelu(outLocal, cv, (float)0.001f, rowCount);
