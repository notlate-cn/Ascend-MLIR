#!/usr/bin/env bash
# New Ascend mainline pipeline for broadcast + add + reducesum.
#
...
echo "========================================================"
echo " broadcast + add + reducesum Ascend mainline pipeline"
echo "========================================================"
echo "shape.M=$M"
echo "shape.N=$N"
...
"$PYTHON" "$DIR/gen_data.py" --m "$M" --n "$N" --seed "$SEED" \
  --out-dir "$BUILD_DIR"
log "  output: input_a.npy input_b.npy output_c.npy"
