...
ARTIFACT_DIR="${ARTIFACT_DIR:-${REPO_ROOT}/build/runtime-mix-leakyrelu}"
DATA_DIR="${DATA_DIR:-${ARTIFACT_DIR}/testdata}"

KERNEL_SRC="${REPO_ROOT}/examples/matmul-add-relu-sum/fc_leakyrelu_mix.cpp"
KERNEL_NAME="${KERNEL_NAME:-fc_leakyrelu}"
...
mapping = [
    ("input_a.npy", input_dir / "fc_leakyrelu_input_a.bin"),
    ("input_b.npy", input_dir / "fc_leakyrelu_input_b.bin"),
    ("input_bias.npy", input_dir / "fc_leakyrelu_input_bias.bin"),
]
for src_name, dst in mapping:
    np.load(npy_dir / src_name).tofile(dst)

golden = np.load(npy_dir / "output.npy")
golden.tofile(output_dir / "fc_leakyrelu_output.bin")
