#!/usr/bin/env bash
# Rank-mixed two-kernel Ascend mainline example.
#
...
TILING_SCHEMA_A="$TILING_SCHEMA_DIR/kernel_a_tiling_space.json"
TILING_SCHEMA_B="$TILING_SCHEMA_DIR/kernel_b_tiling_space.json"
...
echo "========================================================"
echo " rank-mixed two-kernel DAG Ascend mainline pipeline"
echo "========================================================"
echo "shape.N=$N"
echo "shape.K=$K"
...
"$PYTHON" - "$PHASE5_RUNTIME_MANIFEST" "$TILING_SCHEMA_DIR" <<'PY'
import json
import sys
...
if names != ["kernel_a", "kernel_b"]:
    raise SystemExit(f"expected kernel_entries for kernel_a/kernel_b, got {names}")
if len(nodes) != 2:
    raise SystemExit(f"expected two kernelGraph nodes, got {len(nodes)}")
...
print("manifest.kernel_entries=2")
print("manifest.kernelGraph.nodes=2")
PY
...
schema_path, n, k = sys.argv[1], int(sys.argv[2]), int(sys.argv[3])
...
TILING_PARAMS_A="$(build_tiling_params "$TILING_SCHEMA_A")"
TILING_PARAMS_B="$(build_tiling_params "$TILING_SCHEMA_B")"
  ...
  "tasks": [
    {
      ...
      "tiling": {
        "schema": "${TILING_SCHEMA_A}",
        "params": "${TILING_PARAMS_A}"
      },
      "block_dim": ${BLOCK_DIM},
    ...
    },
    {
      ...
      "outputs": [
        { "name": "out", "path": "${ACTUAL_OUTPUT}", "shape": [${N}, ${K}], "dtype": "f16" }
      ],
      "tiling": {
        "schema": "${TILING_SCHEMA_B}",
        "params": "${TILING_PARAMS_B}"
      },
      "block_dim": ${BLOCK_DIM},
      "workspace_size": 16777216,
      "profiling": true
...
grep -v '^\[info\]\|^\[PEM_AIC_LOG\]\|^\[INFO\]\|^\[WARNING\]' \
  "$VALIDATION_LOG" || true
grep -q '^session.plan.tasks=2$' "$VALIDATION_LOG"
...
grep -q '^session.profile.count=2$' "$VALIDATION_LOG"
grep -q '^session.runtime.counter.planned_task_count=2$' "$VALIDATION_LOG"
grep -q '^session.runtime.counter.serialized_launch_count=2$' "$VALIDATION_LOG"

echo ""
echo "========================================================"
echo " rank-mixed two-kernel DAG pipeline complete"
...
echo "   build_mainline/tiling_schemas/kernel_a_tiling_space.json"
echo "   build_mainline/tiling_schemas/kernel_b_tiling_space.json"
