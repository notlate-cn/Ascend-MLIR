...
The model has two independent linalg.generic ops:
  out0 = a + b   (kernel_group0)
  out1 = c * d   (kernel_group1)
...
def main():
    ...
    expected0 = (a.astype(np.float32) + b.astype(np.float32)).astype(np.float16)
    expected1 = (c.astype(np.float32) * d.astype(np.float32)).astype(np.float16)
