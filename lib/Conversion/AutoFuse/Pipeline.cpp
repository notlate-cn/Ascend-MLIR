...
struct AutoFusePipelineOptions
    : PassPipelineOptions<AutoFusePipelineOptions> {
...
void buildAutoFusePipeline(OpPassManager &pm,
                              const AutoFusePipelineOptions &opts) {
  ...
  pm.addNestedPass<func::FuncOp>(createAutoFuseGroupAnalysisPass());

  // Step 3: Outline each group into a private kernel func; optionally split
  // into per-kernel MLIR files.
  std::string outlineSpec = "auto-fuse-group-outline";
  if (!opts.outputDir.getValue().empty())
    outlineSpec += "{output-dir=" + opts.outputDir.getValue() + "}";
  if (failed(parsePassPipeline(outlineSpec, pm)))
    llvm::report_fatal_error("AutoFuse pipeline: failed to add outline pass");
}

} // namespace

void registerAutoFusePipeline() {
  PassPipelineRegistration<AutoFusePipelineOptions>(
      "auto-fuse",
      "AutoFuse pipeline: elewise-fusion → group-analysis → group-outline",
      buildAutoFusePipeline);
  PassPipelineRegistration<>(
      "auto-fuse-codegen",
      "AutoFuse codegen: tile-fuse → bufferize → linalg-to-ascendc → "
      ...
      [](OpPassManager &pm) {
        ...
        pm.addNestedPass<func::FuncOp>(createAutoFuseRestoreMatmulPass());
        ...
        pm.addNestedPass<func::FuncOp>(createAutoFuseIsolateKernelOutputsPass());
        ...
        if (failed(parsePassPipeline(
          ...
          llvm::report_fatal_error(
              "auto-fuse-codegen: failed to add bufferize pass");
        ...
        pm.addNestedPass<func::FuncOp>(createAutoFuseFoldShadowAllocPass());
        pm.addNestedPass<func::FuncOp>(createAutoFuseInsertTileBuffersPass());
