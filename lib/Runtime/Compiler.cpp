...
llvm::Expected<std::string> Compiler::Compile(const std::string& src_file,
  ...
  // Build one bisheng compile command.
  // extra_defs: additional -D flags (used for mix AIC/AIV macros).
  auto makeCompileCmd = [&](const std::string& arch,
  ...
  std::string obj_files; // space-separated list for linker

  if (cfg_.kernel_type == "mix") {
    // Mix: compile twice — AIC (cube) and AIV (vec) — with __MIX_CORE_MACRO__
    ...
    std::string aic_def  = mix_base + " -Dauto_gen_" + kernel_name +
                           "_kernel=" + kernel_name + "_0_mix_aic";
    std::string aiv_def  = mix_base + " -Dauto_gen_" + kernel_name +
    ...
    // vec or cube: single compile with configured arch
  ...
  // Link
  std::string link_cmd = lld + " -m aicorelinux -Ttext=0 " +
                         obj_files + " -static -o " + bin_file;
  if (cfg_.verbose)
