...
__LAYOUT_INIT__  }
  ...
  if ((!home || !*home) && std::getenv("ASCEND_TOOLKIT_HOME"))
    home = std::getenv("ASCEND_TOOLKIT_HOME");
  if (!home || !*home) {
    std::cerr << "ASCEND_HOME_PATH or ASCEND_TOOLKIT_HOME is required\n";
    dlclose(packed);
    freeArrays();
    return 3;
  }
  ...
  if (!hostCannArch.empty())
    body += "  lib_path = home_str + \"/" + hostCannArch +
            "/simulator/\" + soc_ver + \"/lib/libruntime_camodel.so\";\n";
  body += R"cpp(
  void* lib = lib_path.empty() ? nullptr
                               : dlopen(lib_path.c_str(), RTLD_LAZY | RTLD_GLOBAL);
  if (!lib) {
...
int main(int argc, char** argv) {
  ...
  std::string home_str = home;
    ...
    + "  std::string lib_path;\n"
    + (getHostCannArchDir().empty()
           ? std::string()
           : "  lib_path = home_str + \"/" + getHostCannArchDir() +
                 "/simulator/\" + soc_ver + \"/lib/libruntime_camodel.so\";\n")
    + R"cpp(
  void* lib = lib_path.empty() ? nullptr
                               : dlopen(lib_path.c_str(), RTLD_LAZY | RTLD_GLOBAL);
