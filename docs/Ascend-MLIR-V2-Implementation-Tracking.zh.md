...
| Transformer dynamic | Full transformer graph compiles through new mainline | `examples/transformer/run-mainline.sh` proves Normalize + Kernelize prefix, verifies rank-agnostic transpose Kernelize, and records rank3 `linalg.batch_matmul` schedule as the next fail-closed gap | `Transpose Kernelize Generalized` | Add rank3 batch matmul schedule/lowering support |
...
8. Phase 5C+ 已完成显式 multi-kernel DAG runtime manifest、target-aware schedule tile policy、rank-agnostic transpose Kernelize 与 rank2 swap transpose backend lowering；下一步推进 transformer attention rank3 batch matmul 与 compiler-generated kernel DAG metadata
