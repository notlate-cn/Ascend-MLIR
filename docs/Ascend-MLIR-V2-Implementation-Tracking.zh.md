...
| Cross-stage contract hardening | 上层主干不应靠裸字符串 / 重复 enum / 隐式 dialect 注册扩展 | `ascendc.unit`、`AiCore.*`、`ascendc.kernel_kind`、op-role、gather attrs 已集中到 `Common/Attributes.h`；Realize `MemoryPlace` 已统一 alias 到 target profile 枚举；Normalize / Kernelize / Schedule / Realize 已补齐 dependent dialects；Kernelize target op 收集改为 `linalg::LinalgOp` interface 并新增 `linalg.fill` 覆盖测试 | `P0 Slice Closed` | 继续推进 KernelizeOpInterface、Phase5 body classifier 单一来源、Realize plan 驱动 IR mutation |
