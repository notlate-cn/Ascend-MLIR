...
7. `matmul-add-leakyrelu` 默认入口已迁移到新主线，覆盖 dynamic matmul + bias add + leaky_relu 到 CANN mix codegen/runtime sim；下一步推进 full transformer codegen、动态 guard / 多 kernel DAG manifest
