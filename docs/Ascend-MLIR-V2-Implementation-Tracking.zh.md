...
| Phase 3B | Realize materialization 增强 | `Done` | One-Shot Bufferize、target-aware placement plan、workspace layout/lifetime、explicit data movement plan、memory-space annotation MVP、普通 vector output Phase 5 bridge 已完成；selected movement 已支持 local alloc/copy、同形 workspace subview 物化、跨形状 flat workspace packing；StaticMemoryPlanner 已支持非重叠 vector temporary 复用物理 slot offset；完整 producer/consumer view-chain copy 转后续增强 |
...
- Realize selected movement 已支持跨形状 flat workspace packing；完整 producer/consumer view-chain copy 仍是后续增强。
...
| full value-level workspace/copy materialization enhancement | `In Progress` | selected movement 的同形静态 workspace alloc/subview 与跨形状 flat workspace packing 已落地；静态 planner 已支持非重叠 vector temporary slot reuse；更多 producer/consumer view-chain copy materialization 继续作为后续增强 | Phase 3B+ |
