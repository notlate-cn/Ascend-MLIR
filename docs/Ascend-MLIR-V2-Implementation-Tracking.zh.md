...
| Target-driven tile | Tile selection consumes target memory / intrinsic / cost model | `--ascend-schedule='target-tile-policy=target-aware ...'` loads CANN target profile, verifies memory / intrinsic / cost models, and derives tile size from UB capacity plus data-movement path cost availability；tail-policy preference 与 vector buffer count 已进入 `TargetTilePolicy`，`ScheduleDecision` 读取策略对象而不是静态优先序；rank-1 result 按 UB 容量推导 tile，动态 inner extent 显式选择安全 tile 1，不再静默退回 `target_default_32`；stock `--ascend-schedule` 默认改为 `require-explicit`，兼容调用必须显式写 `target-tile-policy=legacy-default`，商用调用必须显式写 `target-aware` | `MVP Closed` | Broaden search beyond vector row tile and migrate selected examples to `target-aware` |
