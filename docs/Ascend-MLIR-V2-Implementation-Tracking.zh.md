...
| supported example default entry migration | `Done` | `broadcast-add-reduce`、`relu-broadcast-transpose`、`add-broadcast-concat` 的默认 `run.sh` 已委托新主线 `run-mainline.sh`；旧 transform-interpreter 入口保留为 `run-legacy.sh` 便于对比 | unsupported examples still legacy：gather/matmul/split |
...
| xvm example smoke follow-up | 删除 broadcast/relu 根目录旧生成物后，`run_simbackend_examples.sh broadcast-add-reduce` passed；`run_simbackend_examples.sh add-broadcast-concat` passed；`add-broadcast-concat/build_mainline` + `broadcast-add-reduce/build_mainline` cross-session smoke passed；完整 `example_pipelines.sh` 仍受既有 `gather-elementwise-fusion` legacy sim abort 影响 |
