...
| Target-driven tile | Tile selection consumes target memory / intrinsic / cost model | `--ascend-schedule='target-tile-policy=target-aware ...'` loads CANN target profile, verifies memory / intrinsic / cost models, and derives tile size from UB capacity plus data-movement path cost availability | `MVP Closed` | Broaden beyond vector row tile and decide when to flip examples from `legacy-default` |
...
| Transformer dynamic | Full transformer graph compiles through new mainline | `examples/transformer/run-mainline.sh` proves Normalize + Kernelize prefix, verifies rank-agnostic transpose Kernelize, and records multi-kernel function schedule metadata as the next fail-closed gap | `Transpose Kernelize Generalized` | Move function-scoped schedule metadata to per-kernel metadata before resuming rank3 batch matmul schedule/lowering |
| Cross-stage contract hardening | 上层主干不应靠裸字符串 / 重复 enum / 隐式 dialect 注册扩展 | `ascendc.unit`、`AiCore.*`、`ascendc.kernel_kind`、op-role、gather attrs 已集中到 `Common/Attributes.h`；Realize `MemoryPlace` 已统一 alias 到 target profile 枚举；Normalize / Kernelize / Schedule / Realize 已补齐 dependent dialects；Kernelize target op 收集改为 `linalg::LinalgOp` interface 并新增 `linalg.fill` 覆盖测试；Phase5 body classifier 已收敛为 `Backend/LinalgBodyClassifier` 单一来源，Realize 与 ComputeLower 共用 | `P0 Slice Closed` | 继续推进 KernelizeOpInterface、Realize plan 驱动 IR mutation |
...
| Schedule function metadata scope | 单 `func.func` 内多个 kernel 不得把第一个 kernel 的 selected tile / tail plan 静默当成全函数事实 | `StructuredLoweringDriver` 在已有函数级 schedule metadata 与当前 kernel metadata 不一致时 fail-closed；同 metadata 的同 kernel 多 op 仍允许复用；transformer smoke 已把该限制记录为当前 next gap | `Closed` | 将函数级 schedule metadata 下沉为 per-kernel / per-entry schema 后恢复多 kernel 单函数后续 lowering |
...
| transformer dynamic smoke | `Done` | `examples/transformer/transformer_dynamic.mlir` 已纳入 Phase 5 验收 smoke；Kernelize 已按 `LayoutTransform` 语义接收 rank3 transpose，backend 仅 lower 当前可支持的 rank2 swap transpose；完整 transformer 当前 fail-closed 在 multi-kernel function schedule metadata，避免函数级 selected tile / tail plan 静默覆盖 | `ascend-phase5-transformer-dynamic-smoke.mlir`；`ascend-kernelize-transpose.mlir`；`ascend-compute-lower-transpose*.mlir`；`ascend-schedule-rejects-multi-kernel-func-metadata.mlir` |
...
| transformer_dynamic transpose closure | `Done` | Kernelize 对 `LayoutTransform` 不再按 rank/permutation 判 supported；backend lowering 通过统一 transpose plan 仅 lower 当前可支持的 rank2 `[1,0]` 到 `ascendc.transpose`，其它形态继续 fail-closed；`run-mainline.sh` 当前 gap 更新为 `multi_kernel_func_schedule_metadata` | `ascend-kernelize-transpose.mlir`；`ascend-compute-lower-transpose.mlir`；`ascend-compute-lower-transpose-unsupported.mlir`；`ascend-phase5-transformer-dynamic-smoke.mlir` |
...
| xvm Schedule multi-kernel func metadata consolidation | `ascend-schedule-rejects-multi-kernel-func-metadata.mlir` RED confirmed old schedule returned status 0 and kept first kernel func attrs；fix 后 focused Schedule / full-pipeline / backend / Phase5 lit 23/23 passed；Ascend Conversion lit 62/62 passed；Target lit 15/15 passed；`examples/transformer/run-mainline.sh` passed：`multi_kernel_func_metadata=fail_closed`、`next_gap=multi_kernel_func_schedule_metadata`；code naming guard passed |
