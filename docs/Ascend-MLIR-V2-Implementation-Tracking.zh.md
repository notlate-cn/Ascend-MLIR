...
| transformer fragment ladder | `Done` | 新增 `examples/transformer-fragments` 小片段阶梯：`layernorm` 覆盖 no-matmul reduction + rsqrt + scale/bias；`qkv` 覆盖 flattened QKV projection `tokens x hidden -> tokens x 3hidden`；两者均从 Normalize / Kernelize / Schedule / Realize / ComputeLower / CANN translate / runtime-session sim 到 named-final-output validation 闭环；runtime-session 构图阶段增加 mix artifact ABI input/output count guard，避免 run manifest 多传 dead buffer 导致 launch 参数错位 | `test/tools/examples/transformer-fragments.mlir`；xvm focused LIT passed：layernorm/qkv `session.validation=pass` |
