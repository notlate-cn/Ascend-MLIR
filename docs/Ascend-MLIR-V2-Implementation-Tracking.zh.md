...
| KernelizeOpRegistry | `Done` | `DependencyAnalysis` 通过 `KernelizeOpRegistry` 默认 linalg model 获取 semantic summary，并在 report 输出 model / trait，后续 op model 集中注册 | `ascend-kernelize-linalg-interface.mlir` |
