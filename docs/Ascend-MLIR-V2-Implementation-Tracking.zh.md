...
| supported example default entry migration | `Done` | `broadcast-add-reduce` 与 `relu-broadcast-transpose` 的默认 `run.sh` 已委托新主线 `run-mainline.sh`；旧 transform-interpreter 入口保留为 `run-legacy.sh` 便于对比 | unsupported examples still legacy：concat/gather/matmul/split |
...
| xvm Phase 5C Ascend Conversion regression | `llvm-lit -v build/test/Conversion --filter="ascend-"` 49/49 passed |
...
| xvm example smoke follow-up | 删除 broadcast/relu 根目录旧生成物后，`run_simbackend_examples.sh broadcast-add-reduce` passed；`add-broadcast-concat/build_e2e` + `broadcast-add-reduce/build_mainline` cross-session smoke passed；完整 `example_pipelines.sh` 仍受既有 `gather-elementwise-fusion` legacy sim abort 影响 |
