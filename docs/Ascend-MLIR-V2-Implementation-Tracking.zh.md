...
- Dynamic workspace expression 计划：`docs/superpowers/plans/2026-05-18-ascend-dynamic-workspace-expression.md`
- Transformer runtime-session E2E 计划：`docs/superpowers/plans/2026-05-18-transformer-runtime-session-e2e.md`
- Transformer runtime E2E closure 计划：`docs/superpowers/plans/2026-05-19-transformer-runtime-e2e-closure.md`
...
| Phase 5C+ | Shape-general Schedule 泛化 | `In Progress` | `axisScheduleConstraints` / `axisCoalescingHints`、role-driven reduction/vector/cube tiling、tail-policy guards、symbolic tile parameter metadata 已落地；cube schedule tile 已覆盖 matmul `[M,N,K]` logical axes，K 轴进入 tail plan，rank3 `batch_matmul` 已有 `[B,M,N,K]` schedule 回归与 A2/B2/CO1 batched `mmad` lowering 回归；Schedule 搜索已扩展到 axis-product / coalescing-hint 候选并加入 cost ranking；`ScheduleDecision` guard ownership 已收敛到 `ScheduleInstance`；StructuredLowering 已发射 guard/tail marker attrs、target tile policy metadata、`tile_binding` / `tile_params` 和 per-kernel schedule metadata；artifact manifest 已支持 compiler-generated multi-kernel DAG、per-kernel schedule metadata 以及 shape/workspace/resource/schema 化 tiling params，workspace size 可由 Realize static planner 回填到 CANN artifact ABI，动态 shape workspace 可通过 `cann.workspace_size_expr` 传播到 tiling-space / manifest / host `_GetWorkspaceSize`；target-aware tile policy 已能消费 CANN target model，gather semantic alignment 已从 `TargetMemoryModel::AlignmentRule` 推导；Schedule tuning signature 已支持 module-level、line-based file cache 与 schema-versioned tuning DB；主线 examples 已迁移为显式 `target-aware` helper；Kernelize 已提供 registry + generated OpInterface 双路径语义契约，linalg/tensor/arith external models 已真实注册；attention_sdpa handwritten contract 已进入 Schedule template；transpose Kernelize 已去 rank/permutation 定制，rank2 swap transpose backend lowering 保持闭环；transformer dynamic mainline 现在通过 `--ascend-kernel-split` 把单函数内 53 个 logical kernel 拆成独立 CANN kernel，C++ kernel、host tiling helper、manifest kernel entry 均为 53；runtime-session E2E 已能自动生成 53-task run manifest，按 per-kernel `kernelKind` 编译 vec/cube/mix artifact，并在默认 `batch=1, seq=1` 下完成并发 simulator validation：`session.result=success`、`session.validation=pass` |
...
| Structured markers | `TailPlanMarker` / guard markers / cache markers are explicit IR carriers | Schedule emits symbolic tile params, guard marker, tail marker, tail policy and target tile policy attrs | `MVP Closed` | Materialize richer marker objects when cache/runtime consumers need them |
...
| Schedule cube K axis | cube schedule 必须覆盖 matmul 的 `[M,N,K]` logical axes，不得只按 result `[M,N]` 生成 tile/tail plan | cube role-driven tile 与 full tile 已改为基于 `CoalescedAxisInfo.logicalAxes`，K 轴保留在 symbolic tile params / tail plan 中；rank2 vector/reduction lowering 消费 kernel-level symbolic tile param 前缀，避免 cube+vector epilogue 因 K 轴元数据失败；rank3 `batch_matmul` 已有 ScheduleSearch `[B,M,N,K]` 轴回归，ComputeLower 对 rank3 A2/B2/CO1 通过 batch loop + tbuf offset slice 降到 `mmad`；on-chip CO1 输出的 `linalg.batch_matmul` 已由 backend classifier 正式归类为 `BatchMatmul`；CANN mix translator 已可发射 rank3 batch-matmul cube loop + full-rank bias-add vector stage，`attn_score` runtime-session sim validation 通过 | `Closed` | 后续扩大到 softmax / full attention 片段和完整 transformer runtime validation |
| Schedule function metadata scope | 单 `func.func` 内多个 kernel 不得把第一个 kernel 的 tile params / tail plan 静默当成全函数事实 | `StructuredLoweringDriver` 已写入 `ascend.schedule.kernel_metadata` per-kernel array；单 kernel / 同 metadata 情况保留 func-level symbolic attrs，多 kernel metadata 不一致时清理 func-level attrs，避免下游读取错误整函数事实；`CannRuntimeArtifacts` 已按 kernel 名消费 per-kernel tile params / tail plan / decision id，func-level attrs 只作为单 kernel fallback | `Closed` | 后续 artifact manifest 扩展 workspace / resource 字段时继续沿用 per-kernel schema |
| Axis static extent consistency | 同一 logical axis 不能在同一 kernel pattern 内静默合并不同静态 extent | `AxisCoalescer` 在同轴静态 extent 冲突时记录 barrier 并发出 op 级错误，避免把第一个 extent 写成错误 guard / tile param | `Closed` | 后续将所有 coalescing barrier 的消费策略统一到 ScheduleProblem |
| Schedule axis identity vs size symbol | `logicalAxisId` 是调度轴 identity，`symbolName` 只表示 extent 等价类；同一个 size symbol 可出现在多根不同 logical axis 上 | `AxisCoalescer` 只在 symbol 对当前 pattern 唯一时用作桥接，重复 symbol 回退 raw axis；`ScheduleAxisContract` 按 `logicalAxisId` 去重/删除 reduction；报告改为 `axisN(sym=...)`；重复 symbol 的 tile param 使用 `T_axisN_<symbol>` 消歧；debug 成员显示 `value_ordinal` | `Closed` | 后续若引入更复杂 reshape/transpose 证明，仍必须保持 axis identity 和 extent symbol 分离 |
...
| all-parallel vector bounded tile | `Done` | `ScheduleSearch` 对 `vector_generic` 生成 role-driven bounded parallel tile，rank2 dynamic/static 大 shape 生成 symbolic tile params；`ascend-compute-lower` 对 rank2 identity 与 rank1 row/col projection all-parallel symbolic tile 物化 `scf.for` / subview / tiled writeback，随后 `--ascend-parallelize` 将外层 tile loop 映射为 `ascendc.get_block_idx`；broadcast-transpose 等 unsupported map 保守回退旧整块 lowering | `ascend-schedule-search.mlir`；`ascend-schedule-vector-bounded-tile.mlir`；`ascend-compute-lower-symbolic-tile.mlir`；`ascend-full-pipeline-rank2-elementwise-add.mlir` |
| Phase 5C+ metadata bridge | `Done` | `ScheduleDecision` 不再复制 instance guard 字段；StructuredLowering 持久化 guard/tail marker attrs 与 target tile policy；target tile 默认策略集中到 `TargetTilePolicy` hook；artifact manifest 保留旧字段并增加 `kernel_entries[0].tilingParams` | `check_ascend_schedule_decision_contract.sh`；`ascend-normalize.mlir`；`ascend-schedule-structured-lowering.mlir`；`ascend-schedule-search.mlir`；`cann-translate-runtime-artifacts.mlir` |
| multi-kernel DAG artifact manifest | `Done` | `ArtifactManifestBuilder` 可为多个 `ascendc.global` kernel 生成 `kernel_entries`，并从 `ascend.kernel_graph.edges` 生成 `kernelGraph`；host tiling 已从单 kernel MVP 扩展为 per-kernel helper emission | `cann-translate-runtime-artifacts-multi.mlir` |
...
| transformer fragment ladder | `Done` | 新增 `examples/transformer-fragments` 小片段阶梯：`layernorm` 覆盖 no-matmul reduction + rsqrt + scale/bias；`qkv` 覆盖 flattened QKV projection `tokens x hidden -> tokens x 3hidden`；`qkv_heads` 覆盖 flat QKV 到 `tokens x 3 x heads x head_dim` 的 reshape/head-split view-chain；`qkv_project_heads` 覆盖单片段 `matmul+bias+identity epilogue -> rank4 heads output`，验证 CANN mix 对 no-activation vector epilogue 与物理 rank2 / 逻辑 rank4 输出 view 的通用处理；`attn_score` 覆盖 rank3 `batch_matmul + full-rank bias add` 的非 toy attention score 片段，并已通过 CANN mix translator、runtime-session compile、sim validation；runtime-session 构图阶段增加 mix artifact ABI input/output count guard，避免 run manifest 多传 dead buffer 导致 launch 参数错位；runtime mix npy shape reconcile 允许输出 golden 按元素数线性化回填物理动态维，同时保留运行 manifest 的逻辑 rank4 validation | `test/tools/examples/transformer-fragments.mlir`；`cann-translate-mix-identity.mlir`；`cann-translate-mix-batch-attn.mlir`；xvm focused LIT passed：layernorm/qkv/qkv_heads/qkv_project_heads/attn_score `session.validation=pass` |
...
| final review | spec review passed；code quality re-review approved：`--ascend-compute-lower` external declaration crash 改为 no-op；artifact manifest `shapeArgOrder.abiPosition` 改为 dense `shape_args` ABI 顺序 |
...
```text
Phase 5: ComputeLoweringDriver -> ABI lowering -> HostTilingEmitter -> ArtifactManifestBuilder -> transformer smoke
```
...
5. `relu-broadcast-transpose` 已新增新主线脚本和 full-pipeline LIT，覆盖 `(d0,d1)->(d1,0)` 常量投影 broadcast/transpose 到 CANN codegen/runtime sim；rank2 identity all-parallel ordinary path 已补充 symbolic tile -> `ascendc.get_block_idx` full-pipeline LIT
...
8. Phase 5C+ 已完成显式 multi-kernel DAG artifact manifest、target-aware schedule tile policy、rank-agnostic transpose Kernelize、rank2 swap transpose backend lowering、multi-kernel function schedule metadata fail-closed、rank3 attention score fragment，以及 transformer 单函数 logical multi-kernel split 到 53 个 CANN kernel 的 artifact 闭环；53-kernel run manifest / per-kernel tiling 已能生成并完成 53 个 artifact 编译；fragment ladder 已关闭 softmax / context / composed attention block，并补齐 GM fill scalar-loop init、单 kernel 多 output ABI 绑定、GM transpose inverse-permutation fallback；multi-func CANN translate 已按 per-function `kernelKind` 发射 mix shell；full transformer `--runtime-e2e --log` 已在默认 `batch=1, seq=1` 下完成 53-task runtime-session simulator validation。下一步把该 gate 扩大到固定 shape matrix、真实 NPU smoke 和更严格的商用前 performance / stability baseline
