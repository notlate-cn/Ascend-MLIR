...
| Transformer dynamic | Full transformer graph compiles through new mainline and has an explicit pre-commercial runtime E2E gate | `examples/transformer/run-mainline.sh` proves Normalize + Kernelize prefix, verifies rank-agnostic transpose Kernelize, and passes full codegen / Phase5 backend / translate / runtime artifact smoke；`--runtime-e2e` mode now generates deterministic inputs, complete output ABI bindings and a runtime-session run manifest, but the focused E2E is intentionally `XFAIL` because CANN artifact compilation still hits unsupported generated AscendC forms: GlobalTensor->GlobalTensor `DataCopy`, residual GM pointer views, scalar `AscendC::Exp(value)`, and double scalar casts inside the aicore function；自动 SDPA-like matcher 已 fail-closed，避免把 MLP 双 matmul 误合成一个 handwritten kernel | `MVP Closed / Runtime E2E Blocked` | 先关闭 transformer runtime-session CANN compile blocker，再进入 runtime sim numeric validation；之后才能讨论完整网络端到端商用 gate |
