...
- Realize 仍未实现跨形状 workspace packing 与完整 producer/consumer view-chain copy。
...
| full value-level workspace/copy materialization enhancement | `In Progress` | selected movement 的同形静态 workspace alloc/subview 已落地；静态 planner 已支持非重叠 vector temporary slot reuse；更多 producer/consumer copy materialization 与跨形状 workspace packing 继续作为后续增强 | Phase 3B+ |
