...
| Transformer dynamic | Full transformer graph compiles through new mainline | `examples/transformer/run-mainline.sh` proves Normalize + Kernelize prefix, verifies rank2 transpose closure, and records rank3 transpose semantics as the next fail-closed gap | `Rank2 Transpose Closed` | Add explicit rank3/rank5 transpose semantics before moving to transformer attention batch matmul |
...
| transformer dynamic smoke | `Done` | `examples/transformer/transformer_dynamic.mlir` 已纳入 Phase 5 验收 smoke；rank2 swap transpose 已纳入 Kernelize / ComputeLower 支持闭环，其它 transpose permutation/rank 保持 fail-closed；完整 transformer 当前停在 rank3 transpose semantics gap | `ascend-phase5-transformer-dynamic-smoke.mlir`；`ascend-kernelize-transpose.mlir`；`ascend-compute-lower-transpose*.mlir` |
...
| transformer_dynamic transpose closure | `Done` | Kernelize / ComputeLower 只支持 rank2 `[1,0]` named/generic transpose；rank3/rank5 transformer transposes 保持 unsupported，不会静默降成无 permutation 语义的 `ascendc.transpose`；`run-mainline.sh` 记录当前 gap 为 `rank3_transpose_semantics` | `ascend-kernelize-transpose.mlir`；`ascend-compute-lower-transpose.mlir`；`ascend-compute-lower-transpose-unsupported.mlir`；`ascend-phase5-transformer-dynamic-smoke.mlir` |
...
8. Phase 5C+ 已完成显式 multi-kernel DAG runtime manifest、target-aware schedule tile policy、rank2 swap transpose closure；下一步推进 rank3/rank5 transpose semantics、transformer attention batch matmul 与 compiler-generated kernel DAG metadata
