...
| Phase 3B | Realize materialization 增强 | `Done` | One-Shot Bufferize、target-aware placement plan、workspace layout/lifetime、explicit data movement plan、memory-space annotation MVP、普通 vector output Phase 5 bridge 已完成；selected movement 已支持 local alloc/copy 与同形 workspace subview 物化；full producer/consumer copy 与 slot reuse 转后续增强 |
...
| Phase 5C+ | Shape-general Schedule 泛化 | `In Progress` | `axisScheduleConstraints` / `axisCoalescingHints`、role-driven reduction/vector/cube tiling、tail-policy guards、schedule tile metadata 已落地；cube schedule tile 已覆盖 matmul `[M,N,K]` logical axes，K 轴进入 tail plan；Schedule 搜索已扩展到 axis-product / coalescing-hint 候选；`ScheduleDecision` guard ownership 已收敛到 `ScheduleInstance`；StructuredLowering 已发射 guard/tail marker attrs、target tile policy metadata 和 per-kernel schedule metadata；runtime manifest 已支持 compiler-generated multi-kernel DAG 与 per-kernel schedule metadata；target-aware tile policy 已能消费 CANN target model，gather semantic alignment 已从 `TargetMemoryModel::AlignmentRule` 推导；主线 examples 已迁移为显式 `target-aware` helper；transpose Kernelize 已去 rank/permutation 定制，rank2 swap transpose backend lowering 保持闭环，transformer dynamic 当前 fail-closed 到 batch_matmul schedule/lowering |
...
| 项 | 状态 | 主要产物 | 验证 |
|---|---|---|---|
| Schedule 搜索空间扩展 | `Done` | `ScheduleSearch` 增加 axis-product 与 coalescing-hint tile candidates，`generated_candidates` 覆盖扩大 | `ascend-schedule-search.mlir` |
| Schedule cache / report 正确化 | `Done` | `selectedDecisionEntries`、`guardBudgetPruned`、`negativeCacheHits` 分离；只统计选中 decision | `ascend-schedule-cache.mlir` |
...
| Runtime / manifest 消费 per-kernel schedule metadata | `Done` | `CannRuntimeArtifacts` 按 kernel 名读取 `ascend.schedule.kernel_metadata` 的 tile/tail/decision id | `cann-translate-runtime-artifacts-multi.mlir` |
| Realize selected movement IR materialization | `Done` | `materializeMovementSteps` 在 memory-space materialize 生产路径插入 `VECIN` local alloc/copy；movement 先于 annotation，避免 plan source 被 VECCALC 重写 | `ascend-realize-workspace-layout.mlir` |
| KernelizeOpRegistry | `Done` | `DependencyAnalysis` 通过 `KernelizeOpRegistry` 默认 linalg model 获取 semantic summary，后续 op model 集中注册 | `ascend-kernelize-linalg-interface.mlir` |
| HandwrittenPattern 注入器 | `Done` | `ascend.kernelize.handwritten_group` 生成 `HandwrittenPattern` candidate，统一走 closure / contract / partition | `ascend-kernelize-handwritten-pattern.mlir` |
| 文档收敛 | `Done` | 本节与 Gap Board 更新，记录本批次边界和验证项 | 本文档 |

本批次后仍保留的非目标：

- Realize 仍未实现跨形状 workspace packing、slot reuse、完整 producer/consumer view-chain copy。
- Schedule 仍是候选枚举增强，不是持久化 auto-tuning cache。
- `KernelizeOpRegistry` 是内部 registry；正式 external `KernelizeOpInterface` / trait model 后续单独演进。
- HandwrittenPattern 当前提供注入通道；FlashAttention/SDPA 具体 matcher 与专用 schedule/lowering 不在本批次。
...
| workspace layout / lifetime MVP | `Done` | `StaticMemoryPlanner` 对 on-chip placement 生成保守 `workspace_layout`：一 vector temporary 一个 live interval / workspace slot，slot place 为 `VECIN`，报告 peak usage unit；静态 tensor byte facts 与稳定 value-level facts 已从 `BufferizationDriver` 进入 `BufferizedKernelIR`，可报告 local/workspace/peak byte usage；target-aware 模式已用 `TargetMemoryModel` 校验 `VECIN` peak byte capacity；slot reuse、真实 workspace alloc/subview 后续推进 | `ascend-realize-workspace-layout.mlir`；`AscendRealizePlannerTest` |
| explicit data movement plan MVP | `Done` | `MovementPlanner` 对 on-chip workspace 场景生成 `movement_planning`：记录 value-level movement step、movement demand、workspace reuse candidate；target-aware 模式用 `TargetMemoryModel` 选择 `GM -> VECIN` direct path，缺 direct path 时 fail-closed 保持 deferred；selected direct input movement 已可物化为 local alloc + copy，静态同形同 block 输入会合并到一个 workspace base 并通过 rank-reduced subview 分配 slot；producer/consumer 全量 copy 与 slot reuse 后续推进 | `ascend-realize-data-movement-plan.mlir`；`AscendRealizePlannerTest` |
...
| full value-level workspace/copy materialization enhancement | `In Progress` | selected movement 的同形静态 workspace alloc/subview 已落地；更多 producer/consumer copy materialization、跨形状 workspace layout 和 slot reuse 继续作为后续增强 | Phase 3B+ |
