...
| 4 | **Phase 2-B tile-fuse**         | [1_vector-plan-tile-fuse.mlir](pass_dumps_phase2_kernel_group0/builtin_module_no-symbol-name/1_vector-plan-tile-fuse.mlir)                                       | 每个 feasible TilePlanDraft 复制出一个 `…__v<i>`；挂 `vector_plan.tile_plan` 和 `afir.axis_extent_expr`                    |
