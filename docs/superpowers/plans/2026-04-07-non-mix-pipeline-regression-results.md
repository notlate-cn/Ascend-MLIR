...
- Controller-side xvm rerun after the `Executor.cpp` boundary fix confirmed the effect: the four former environment-blocked examples now initialize the runtime, execute the plain path, and pass validation, while `broadcast-add-reduce` remains the lone compile-time failure at `STAGE 5 --linalg-to-ascendc`.
...
| gather-elementwise-fusion | vec | PASS | none | yes | yes | yes | yes | n/a | xvm@orb / 2026-04-07 follow-up; simulator reached validation and passed with `max_abs_diff=1.494141e+00`, `mean_abs_diff=4.083161e-01`. |
...
| split-relu-brc-add-mul | vec | PASS | none | yes | yes | yes | yes | n/a | xvm@orb / 2026-04-07 follow-up; simulator reached validation and passed with `max_abs_diff=9.765625e-04`, `mean_abs_diff=2.629566e-05`. |
