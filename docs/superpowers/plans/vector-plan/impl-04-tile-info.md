...
**设计依据**: `docs/superpowers/specs/2026-04-14-vector-plan-unified-design.md` §7 §10 §11  
...
void verifyTileInfoInvariants(const TileInfo &ti) {
  ...
  // CubeGroup：必须有 2 个
  if (ti.groupId >= 0 /* CubeGroup 标记 */)
    assert(ti.blockDimExprs.size() == 2 && "CubeGroup must have 2D blockDimExprs");
}
```
