...
**设计依据**: `docs/superpowers/specs/2026-04-14-vector-plan-unified-design.md` §3  
...
SmallVector<AxisInfo> computeCanonicalAxes(ArrayRef<linalg::LinalgOp> members) {
  ...
  // 2. 对每个轴做 join
  SmallVector<AxisRole> roles(maxLoops, /* Absent 用 optional 表示 */);
  SmallVector<bool>     seen(maxLoops, false);
  ...
  for (auto op : members) {
    ...
    for (auto [a, type] : enumerate(iterTypes)) {
      ...
      if (!roles_initialized[a]) {
        roles[a] = cur;
        roles_initialized[a] = true;
      } else {
