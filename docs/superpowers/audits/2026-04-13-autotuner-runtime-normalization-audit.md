...
| Runtime library still compiles retained legacy implementation units | `lib/Runtime/CMakeLists.txt` still builds `Legacy/CompatRuntime.cpp` and `Legacy/Compiler.cpp`. | Autotuner is not the direct reason those units remain, but its runtime path still links the runtime library that contains them. |
