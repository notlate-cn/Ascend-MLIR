...
| Runtime library still compiles retained legacy execution units | `lib/Runtime/CMakeLists.txt` still builds `Legacy/CompatRuntime.cpp`, `Legacy/Compiler.cpp`, `Legacy/Executor.cpp`, and `Legacy/HostRunnerGen.cpp`. | Autotuner is not the direct reason those units remain, but its runtime path still links the runtime library that contains them. |
...
Autotuner does not appear to block `Legacy/HostRunnerGen` cleanup directly. It also no longer owns a direct `Legacy/Executor` or `Legacy/SimValidator` seam; the runtime execution path now goes through `NativeExecutionRunner`, while `ExecutionSession` remains only the dispatcher/orchestration layer.
...
- Re-audit the remaining retained `Legacy/Executor` and `Legacy/Compiler` units; autotuner no longer blocks execution-seam cleanup directly.
