...
| Runtime library still compiles retained legacy implementation units | `lib/Runtime/CMakeLists.txt` still builds `Legacy/CompatRuntime.cpp`, `Legacy/Compiler.cpp`, and `Legacy/HostRunnerGen.cpp`. | Autotuner is not the direct reason those units remain, but its runtime path still links the runtime library that contains them. |
...
- Re-audit the remaining retained `Legacy/Compiler` unit and compatibility helpers; autotuner no longer blocks execution-seam cleanup directly.
