...
**Depends on:** [00-architecture.md](./00-architecture.md) §7, [04-tile-info.md](./04-tile-info.md)
...
## 7. AutoTuner：Solver 接口与搜索空间转换

> [!NOTE]
> 本章定义 **Solver 抽象接口**和 `tiling.infos` → `tiling_space.json` 的转换规则。
> Runner 执行池、剪枝策略和 Fallback 机制详见 [06-autotuner-design.md](./06-autotuner-design.md)。
