...
// CHECK: if (_afir_ds[0] < 16u && _afir_ss[1] == 1u) {
// CHECK: Duplicate({{.*}}, _afir_zero, _afir_ds[0] * _afir_ds[1]);
// CHECK: for (uint32_t _afir_r = 0; _afir_r < _afir_ds[0]; ++_afir_r) {
// CHECK: auto _afir_v = {{.*}}.GetValue(_afir_r);
// CHECK: AscendC::Adds({{.*}}[_afir_r * _afir_ds[1]], {{.*}}[_afir_r * _afir_ds[1]], _afir_v, (int32_t)_afir_ds[1]);
// CHECK: } else {
