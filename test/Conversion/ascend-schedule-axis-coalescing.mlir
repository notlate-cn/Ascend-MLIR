// RUN: sed -n '/\/\/ SINGLE-BEGIN/,/\/\/ SINGLE-END/p' %s | ascend-mlir-opt --ascend-normalize --ascend-kernelize --ascend-schedule='target-tile-policy=legacy-default dump-report=true debug-stage=schedule' 2>&1 | FileCheck %s
// RUN: sed -n '/\/\/ MULTI-PRIMARY-BEGIN/,/\/\/ MULTI-PRIMARY-END/p' %s | ascend-mlir-opt --ascend-normalize --ascend-kernelize --ascend-schedule='target-tile-policy=legacy-default dump-report=true debug-stage=schedule' 2>&1 | FileCheck %s --check-prefix=MULTI
// RUN: sed -n '/\/\/ MULTI-CUBE-BEGIN/,/\/\/ MULTI-CUBE-END/p' %s | ascend-mlir-opt --ascend-schedule='target-tile-policy=legacy-default dump-report=true debug-stage=schedule' 2>&1 | FileCheck %s --check-prefix=CUBE
// RUN: sed -n '/\/\/ CONSTANT-PROJECTION-BEGIN/,/\/\/ CONSTANT-PROJECTION-END/p' %s | ascend-mlir-opt --ascend-schedule='target-tile-policy=legacy-default dump-report=true debug-stage=schedule' 2>&1 | FileCheck %s --check-prefix=CONST
// RUN: sed -n '/\/\/ GATHER-TAIL-BEGIN/,/\/\/ GATHER-TAIL-END/p' %s | ascend-mlir-opt --ascend-schedule='target-tile-policy=legacy-default dump-report=true debug-stage=schedule' 2>&1 | FileCheck %s --check-prefix=TAIL
// RUN: sed -n '/\/\/ EMBEDDING-TAIL-BEGIN/,/\/\/ EMBEDDING-TAIL-END/p' %s | ascend-mlir-opt --ascend-schedule='target-tile-policy=legacy-default dump-report=true debug-stage=schedule' 2>&1 | FileCheck %s --check-prefix=EMBED
// RUN: sed -n '/\/\/ PERMUTED-GATHER-TAIL-BEGIN/,/\/\/ PERMUTED-GATHER-TAIL-END/p' %s | ascend-mlir-opt --ascend-schedule='target-tile-policy=legacy-default dump-report=true debug-stage=schedule' 2>&1 | FileCheck %s --check-prefix=PERMUTE
...
// TARGET-GATHER-BEGIN
#target_gather_col = affine_map<(d0, d1) -> (d1)>
#target_gather_id = affine_map<(d0, d1) -> (d0, d1)>
func.func @manual_target_gather_tail_contract(%data: tensor<?x?xf32>,
                                              %indices: tensor<?xi64>,
                                              %m: index,
                                              %k: index) -> tensor<?x?xf32> {
  %empty = tensor.empty(%m, %k) : tensor<?x?xf32>
  %out = linalg.generic {
      indexing_maps = [#target_gather_col, #target_gather_id],
      iterator_types = ["parallel", "parallel"]
    } ins(%indices : tensor<?xi64>)
...
// TARGETGATHER: AxisCoalescing:
// TARGETGATHER-NEXT: kernel = kernel_0
// TARGETGATHER-NEXT: logical_axes = 2
// TARGETGATHER-NEXT: parallel_axes = [0, 1]
// TARGETGATHER-NEXT: reduction_axes = []
// TARGETGATHER-NEXT: broadcast_axes = [0]
// TARGETGATHER-NEXT: barriers = 0
// TARGETGATHER-NEXT: axis_constraints = [
