...
`vector-plan-group-analysis` 和 `vector-plan-group-outline` 是严格串联的两个 pass：前者只负责给 linalg op 打 `vector_plan.*` 属性，后者消费这些属性并把 group outline 成 kernel func。
...
- 输出中不含 `vector_plan.group_id/topo_index`
...
- 统一使用 `afir-opt --vector-plan-group-analysis --vector-plan-group-outline`
...
```bash
afir-opt --vector-plan-group-analysis --vector-plan-group-outline \
  test/Conversion/VectorPlan/group-pipeline-vertical.mlir
```
...
afir-opt --vector-plan-group-analysis --vector-plan-group-outline \
  test/Conversion/VectorPlan/group-pipeline-vertical.mlir \
  | FileCheck test/Conversion/VectorPlan/group-pipeline-vertical.mlir
```
...
```bash
afir-opt --vector-plan-group-analysis --vector-plan-group-outline \
  test/Conversion/VectorPlan/group-pipeline-single.mlir
```
...
```bash
afir-opt --vector-plan-group-analysis --vector-plan-group-outline \
  test/Conversion/VectorPlan/group-pipeline-horizontal.mlir
```
...
```bash
afir-opt --vector-plan-group-analysis --vector-plan-group-outline \
  test/Conversion/VectorPlan/group-pipeline-no-fuse-fanout.mlir
```

### 4. 只看 GroupAnalysis 标记后的 IR

如果想单独观察 `vector_plan.group_id` / `vector_plan.topo_index`，只跑 analysis pass：
...
for f in \
  ...
  test/Conversion/VectorPlan/group-pipeline-vertical.mlir \
  test/Conversion/VectorPlan/group-pipeline-horizontal.mlir \
  test/Conversion/VectorPlan/group-pipeline-no-fuse-fanout.mlir; do
  echo "===== $f ====="
  afir-opt --vector-plan-group-analysis --vector-plan-group-outline "$f"
done
```
...
```bash
for f in test/Conversion/VectorPlan/group-pipeline-*.mlir; do
  echo "===== $f ====="
  afir-opt --vector-plan-group-analysis --vector-plan-group-outline "$f" \
    | FileCheck "$f"
done
