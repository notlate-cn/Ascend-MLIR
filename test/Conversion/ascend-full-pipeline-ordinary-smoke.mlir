// RUN: sed -n '/\/\/ POSITIVE-BEGIN/,/\/\/ POSITIVE-END/p' %s | afir-opt --ascend-normalize --ascend-kernelize --ascend-schedule='target-tile-policy=legacy-default' --ascend-realize='materialization-mode=memory-space-annotate dump-report=true debug-stage=realize' --ascend-compute-lower 2>&1 | FileCheck %s
// RUN: sed -n '/\/\/ POSITIVE-BEGIN/,/\/\/ POSITIVE-END/p' %s | afir-opt --ascend-normalize --ascend-kernelize --ascend-schedule='target-tile-policy=legacy-default' --ascend-realize='placement-mode=target-aware cann-root=%S/Inputs/ascend-target-aware-placement-cann soc=SyntheticSoC materialization-mode=memory-space-annotate' --ascend-compute-lower --ascend-parallelize --ascend-prepare-for-emit --ascend-canonicalize-cann-signature | FileCheck %s --check-prefix=ABI
// RUN: sed -n '/\/\/ POSITIVE-BEGIN/,/\/\/ POSITIVE-END/p' %s | afir-opt --ascend-normalize --ascend-kernelize --ascend-schedule='target-tile-policy=legacy-default' --ascend-realize='materialization-mode=memory-space-annotate' | FileCheck %s --check-prefix=REALIZE
// RUN: sed -n '/\/\/ NON-IDENTITY-BEGIN/,/\/\/ NON-IDENTITY-END/p' %s | afir-opt --ascend-realize='materialization-mode=memory-space-annotate dump-report=true debug-stage=realize' 2>&1 | FileCheck %s --check-prefix=NON-IDENTITY
// RUN: sed -n '/\/\/ SUBVIEW-BEGIN/,/\/\/ SUBVIEW-END/p' %s | afir-opt --ascend-realize='materialization-mode=memory-space-annotate dump-report=true debug-stage=realize' 2>&1 | FileCheck %s --check-prefix=SUBVIEW
// RUN: sed -n '/\/\/ CAST-BEGIN/,/\/\/ CAST-END/p' %s | afir-opt --ascend-realize='materialization-mode=memory-space-annotate dump-report=true debug-stage=realize' 2>&1 | FileCheck %s --check-prefix=CAST
