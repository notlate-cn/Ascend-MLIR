// RUN: sed -n '/\/\/ POSITIVE-BEGIN/,/\/\/ POSITIVE-END/p' %s | ascend-mlir-opt --ascend-normalize --ascend-kernelize --ascend-schedule='target-tile-policy=legacy-default dump-report=true debug-stage=schedule' 2>&1 | FileCheck %s
// RUN: sed -n '/\/\/ MATMUL-BEGIN/,/\/\/ MATMUL-END/p' %s | ascend-mlir-opt --ascend-normalize --ascend-kernelize --ascend-schedule='target-tile-policy=legacy-default dump-report=true debug-stage=schedule' 2>&1 | FileCheck %s --check-prefix=MATMUL
// RUN: sed -n '/\/\/ MISSING-KERNEL-BEGIN/,/\/\/ MISSING-KERNEL-END/p' %s | not ascend-mlir-opt --ascend-schedule='target-tile-policy=legacy-default' 2>&1 | FileCheck %s --check-prefix=MISSING-KERNEL
