...
// Decomposed LayerNorm subgraph (the form torch lowers nn.LayerNorm to at the
// linalg level), normalizing over the last dim:
//
//   mean   = sum(x)/N                       (reduction + divf)
//   cen    = x - mean                       (subf)
//   var    = sum(cen*cen)/N                 (mul + reduction + divf)
//   rstd   = rsqrt(var + eps)               (addf + rsqrt)
//   norm   = cen * rstd                      (mulf)
//   out    = norm * gamma + beta            (mulf + addf, gamma/beta rank-1)
//
// recognize-layernorm anchors on the rsqrt and folds the whole region into a
// single @__aclnn_layer_norm(x, gamma, beta) call, leaving the generics dead.
...
func.func @ln(%x: tensor<2x4x8xf32>, %gamma: tensor<8xf32>,
              %beta: tensor<8xf32>) -> tensor<2x4x8xf32> {
...
// CHECK: func.func private @__aclnn_layer_norm
// CHECK: aclnn.kind = "layer_norm"
// CHECK-LABEL: func.func @ln
// x, gamma, beta are cast to the fully-dynamic declared type, then called.
// CHECK: call @__aclnn_layer_norm
