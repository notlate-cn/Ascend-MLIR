...
// RUN: afir-opt %s --ascend-normalize --ascend-kernelize --ascend-schedule='target-tile-policy=legacy-default tuning-db-in=%t.seed.db tuning-db-out=%t.out.db dump-report=true debug-stage=schedule' 2>&1 | FileCheck %s --check-prefix=CACHE
