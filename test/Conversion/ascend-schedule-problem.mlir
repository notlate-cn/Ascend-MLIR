// RUN: sed -n '/\/\/ MAIN-BEGIN/,/\/\/ MAIN-END/p' %s | afir-opt --ascend-normalize --ascend-kernelize --ascend-schedule='dump-report=true debug-stage=schedule' 2>&1 | FileCheck %s
// RUN: sed -n '/\/\/ HORIZONTAL-BEGIN/,/\/\/ HORIZONTAL-END/p' %s | afir-opt --ascend-normalize --ascend-kernelize --ascend-schedule='dump-report=true debug-stage=schedule' 2>&1 | FileCheck %s --check-prefix=HORIZONTAL
...
func.func @matmul_problem(%lhs: tensor<4x8xf32>,
